-- Recollection production data model. Run in a new Supabase project's SQL editor.
...
-- Private storage: browser uploads only to its own folder; signed URLs are issued server-side for viewers.
...
-- Guest links are deliberately resolved only by the server using the service role.
-- This keeps raw invitation tokens out of client-side database policies.
